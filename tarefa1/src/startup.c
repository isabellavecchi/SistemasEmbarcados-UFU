...
// para armazenar a Tabela de Vetores de Interrupção na seção .isr_vectors
uint32_t vectors[] __attribute__((section(".isr_vectors"))) =
{
  //stack pointer (SP)
  STACK_START,                            /* 0x0000 0000 */

  /* Esta rotina será responsável por copiar o conteúdo da seção
  .data (Initialized Data) da memória FLASH para a SRAM e inicializar
  a seção .bss (Block Started by Symbol) em zero:
  */
...
extern uint32_t _etext;   /* Endereco de carga na RAM da secao .data */
...
void reset_handler(void)
/*
* Esta função deve, no mínimo, realizar as seguintes tarefas:
* 
...
void default_handler(void)
{
  // n lida com nada, só trava a app caso a exp ocorra
  while(1){};
}